...
export const getAllWarehouses = async (req, res) => {
  try {
    const warehouses = await Warehouse.find({})
      ...
      .populate("products.product", "name price image stock");
...
export const getWarehouseById = async (req, res) => {
  try {
    const warehouse = await Warehouse.findById(req.params.id)
      .populate("manager", "name email")
      .populate("products.product", "name price stock image");
...
export const addStockToWarehouse = async (req, res) => {
  try {
    ...
    // Increment main product schema stock count as well
...
export const transferStock = async (req, res) => {
  try {
    ...
    fromWarehouse.currentStock = fromWarehouse.products.reduce((sum, p) => sum + p.quantity, 0);
    toWarehouse.currentStock = toWarehouse.products.reduce((sum, p) => sum + p.quantity, 0);
